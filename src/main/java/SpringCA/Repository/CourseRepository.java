...
public interface CourseRepository extends JpaRepository<Course, Integer> {
    ...
    @Query(value = "select c from course c where course_id not in (select c.course_id from course c " +
            "inner join student_course sc on c.course_id = sc.course_id inner join student s on " +
            "sc.student_id = s.student_id where s.student_id = :studentId and sc.status in ('Approved', 'Pending'))")
